...
import { ProfileReducer, actions } from "./profileSlice"
...
describe("Profile reducer", () => {
  it("toggle test", () => {
    ...
    expect(ProfileReducer(state as ProfileSchema, actions.toggleReadOnly())).toEqual(
      result,
    )
  })
  ...
  it("cancle edit возвращает editedProfile к исходному(profile) значению", () => {
    ...
    expect(ProfileReducer(state as ProfileSchema, actions.cancleEdit())).toEqual({
      data: {
  ...
  it("профиль редактируется", () => {
    ...
    expect(
      ProfileReducer(state as ProfileSchema, actions.editProfile(expectetResult)),
